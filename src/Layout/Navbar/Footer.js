  ...
  return (
    ...
    <div className="grid-cols-1 grid sm:grid-cols-2 md:grid-cols-5 lg:grid-cols-9 xl:grid-cols-12 gap-5 sm:gap-7 md:gap-9 lg:gap-10 xl:gap-12 py-6 justify-between">
      {Footer.map((text, index) =>(
        ...
        <h3 className="lg:leading-7 text-bold font-bold text-white mb-4 sm:mb-5 lg:mb-6 pb-0.5">{text.title}</h3>  
        <ul className="flex flex-col space-y-3">
          {text.links.map((link, index) =>(
            <li key={index} className="flex items-baseline">
        <links to={link.url} className="text-brd w-full text-sm font-medium inline-block hover:text-sub">{link.name}</links>
        </li>
        ))}
        </ul>
      </div>
